///disini gw hanya sekedar recode:v
///thanks you nayla<3
///thank you Dittaz<3
///takim kont
///mau colong case? Tambah Thanks too NaylaChan
const {
		WAConnection,
		MessageType,
		Presence,
...
/*=====================================================[ CONNECTING  ]==============================================================*/                  	    
        	...
        	nayla.on('qr', () => {
     		console.log(color('[','white'), color('!','red'), color(']','white'), color('FOLLOW GITHUB ZANSLORD'))
	        })
...
/*=====================================================[ API VIDEFIKRI ]==============================================================*/                  	                 
            ...
            const tomxic = ["ajg","asu","Ajg","Asu","anjg","Anjg","olol","antek","elaso","Babi","babi","ontol","kntl","kintil","emek","entod","puqi","kimak","telaso","lanjut","memeg","pukimak"]
            ...
            */const vnayla = 'BEGIN:VCARD\n' 
            ...
            + `FN: ZANSLORD\n` 
            + `ORG: AUTHOR;\n`
            + `TEL;type=CELL;type=VOICE;waid=:+62 858-6907-4622\n`
            + 'END:VCARD' 
...
/*=====================================================[ API FREEEEEE ]==============================================================*/                  	    
                    ...
                    nayla.sendMessage(`${setting.ownerNumber}@s.whatsapp.net`, optionsp, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "caption": `FOLLOW GITHUB ZANSLORD`} } } })                    
                    reply('BOT AKAN SEGERA MASUK. FOLLOW GITHUB ZANSLORD')                     
					break  
					case 'claim':
			        ...
			        inf1 += `FOLLOW GITHUB *https://github.com/ZansLord`
			        reply(inf1)
			        break
					case 'elitebot': 				
...
/*=====================================================[ API VIDEFIKRI ]==============================================================*/                  	    
                   ...
                   nayla.sendMessage(from, anu, image, {caption: `nihh coeg`, quoted: nay1})
                   break
                   case 'google':  
                   if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			       if (isLimit(sender)) return
...
/*====================================================[ API?? JOJO API ]==============================================================*/                  	    
                   ...
                   nayla.sendMessage(from, anu1, image, {caption: `nihh coeg`, quoted: nay1})
                   break
                   case 'qrcode':  
                   if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			       if (isLimit(sender)) return
...
/*======================================================[ API?? ZEKS ]==============================================================*/                  	    
                   ...
                   anu1 = await getBuffer(anu.result)
                   nayla.sendMessage(from, anu1, image, {caption: `nihh mimim`, quoted: nay1})
                   break
                   case 'darkjokes':  
                   if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			       if (isLimit(sender)) return
                   ...
                   anu1 = await getBuffer(anu.result)
                   nayla.sendMessage(from, anu1, image, {caption: `Ngedark terus ngab:v`, quoted: nay1})
                   break
                   case 'nickff':  
                   if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			       if (isLimit(sender)) return
                   ...
                   F = body.slice(6)
                   reply(naylachan)
                   anu = await getBuffer(`https://api.zeks.xyz/api/wolflogo?apikey=apivinz&text1=KABANERIbotz&text2=${F}`)
                   nayla.sendMessage(from, anu, image, {caption: `nihh coeg`, quoted: nay1})
                   break 
                   case 'tfire':  
                   if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
...
/*====================================================[ API?? NOT API ]==============================================================*/                  	    
			        ...
			        nayla.sendMessage(from, buffer, image, {quoted: nay, caption: 'nihh coeg'})
				    fs.unlinkSync(ran)
					})					
			    	break 
                    case 'readmore':  
					...
					.on('end', function () {
					console.log('SELESAI JANGAN LUPA SUBSCRIBE YT KABANERIBOTZ')
...
/*=====================================================[ CASE ANTIII ]==============================================================*/                  	    
                    ...
                    nayla.sendMessage(from, anu, video, {caption: `nihh coeg`, quoted: nay1}) 
                    break
                    case 'video2':  
                    if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			        if (isLimit(sender)) return
...
/*=======================================================[ API XTEAM ]==============================================================*/                  	    
                    ...
                    case 'gon': case 'killua': case 'sagiri': case 'KABANERI': case 'natsu': case 'genos': case 'saitama': case 'miku':  
                    if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			        if (isLimit(sender)) return
			        await limitAdd(sender)
                    nayla.updatePresence(from, Presence.composing) 
...
/*====================================================[ API?? LINDOWAPI ]==============================================================*/                  	    
                     ...
                     nayla.sendMessage(from, anu, image, {caption: `nihh coeg`, quoted: nay})
                     break
                     case 'exo':  
                     if (!isElite) return reply(nyz.nayzelite(pushname, prefix))
			         if (isLimit(sender)) return
...
/*=====================================================[ API VHTEAR ]==============================================================*/                  	    
				    ...
				    nayla.sendMessage(from, to, image, {caption: 'nihh coeg', quoted:nay1})
				    break
...
/*=====================================================[ ANTI RANDOM ]==============================================================*/                  	    
					...
					if (isGroupAdmins) return reply('Alahh Siahh Euyy Admin Grup Mah Bebas😎 Punteun Ah')
					nayla.updatePresence(from, Presence.composing)
					...
					if (!isGroup) return
					if (!isAntigay) return
					if (isGroupAdmins) return reply('Alahh Siah Boyy Admin Grup Mah Bebas Rek Ngagay Oge😎 Ezzz Brad')
					nayla.updatePresence(from, Presence.composing)
					if (messagesC.includes("@62812874133914")) return reply("izin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
					...
					if (isGroupAdmins) return reply('Alahh Siahh Brad Admin Mah Bebas Rek Bocil oge😎 Eazzz Brader')
					nayla.updatePresence(from, Presence.composing)
					if (messagesC.includes("@62812874133914")) return reply("izin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
					...
					if (isGroupAdmins) return reply('Alahh Siahh Brodi Admin Mah Bebas Rek Wibu Oge😎 Ezzz Brodi')
					nayla.updatePresence(from, Presence.composing)
					if (messagesC.includes("@62812874133914")) return reply("izin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
